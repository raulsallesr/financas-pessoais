...
convergência. `focuslens/adapters/mobile_snapshot.py` entrega somente contratos
já calculados em
...
não substitui `focuslens/core/curva_cenarios.py`, não calcula preço ou retorno e
não entra nos
...
`python -m scripts.gerar_mobile_snapshot` lê somente os dois caches públicos
versionados, sem
...
adaptador de formato já testado em `focuslens/adapters/b3_importacao.py`:
reconhece as seis abas
...
métricas estão em
[`ESTRATEGIA_INSTITUCIONAL.md`](../product/ESTRATEGIA_INSTITUCIONAL.md).
...
- o gate físico do cofre está em
  [`VALIDACAO_CARTEIRA_LOCAL.md`](../validation/VALIDACAO_CARTEIRA_LOCAL.md).
