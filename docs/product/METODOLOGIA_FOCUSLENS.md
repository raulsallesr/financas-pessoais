...
- `focuslens/core/resumo_integrado.py`: prioridade, provas, datas, limites e condições;
- `focuslens/ui/pagina_resumo.py`: apresentação e divulgação progressiva do método;
- `focuslens/core/curva_cenarios.py`: choque paralelo puro e guardrails;
- `focuslens/ui/pagina_home.py`: jornada Resumo → Expectativas → Curva → Carteira;
